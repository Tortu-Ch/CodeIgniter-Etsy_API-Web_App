...
if (!function_exists('PT_ShortText')) {
    function PT_ShortText($text = "", $len = 20)
    {
...
if (!function_exists('etsyApi_makeRequest')) {
    function currencyConverter($from_currency, $to_currency) {
//        https://free.currconv.com/api/v7/convert?apiKey=3e1a52dcab4f1d0d86ca&q=AUD_USD&compact=y
        $url = "https://free.currconv.com/api/v7/convert?apiKey=3e1a52dcab4f1d0d86ca&q=";
        $url .= $from_currency.'_'.$to_currency.'&compact=y';
        $ch = curl_init($url);
...
if (!function_exists('etsyApi_makeRequest')) {
    function etsyApi_makeRequest($url, $callDetails = false)
    {
        // Set handle
        $ch = curl_init($url);
